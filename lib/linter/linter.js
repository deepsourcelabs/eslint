...
// marvin-js >
/**
 * Checks whether a given AST belongs to a generated file
 * @param {ExtendedProgam} ast The AST to check
 * @returns {boolean} true if the AST seems to be from a generated file
 * @throws
 */
function isGeneratedAST(ast) {
    const TOKEN_DENSITY_THRESHOLD = 150;
    const IDENTIFIER_THRESHOLD = 40;
    const LOC_THRESHOLD = 20000;

    const { loc } = ast;

    if (!loc) {
        throw new Error("Location information not found in the AST");
    }

    const { tokens } = ast;

    if (!tokens) {
        throw new Error("Token information not found in the AST");
    }

    const linesOfCode = (loc.end.line - loc.start.line) || 1;
    const tokenDensity = tokens.length / linesOfCode;

    let identifierCount = 0;
    let singleCharIdentifierCount = 0;

    for (const token of tokens) {
        if (token.type === "Identifier") {
            identifierCount++;
            if (token.value.length === 1) {
                singleCharIdentifierCount++;
            }
        }
    }

    const singleCharIdentifierPercent = (singleCharIdentifierCount / identifierCount) * 100;
    const isGenerated =
        !!(tokenDensity > TOKEN_DENSITY_THRESHOLD ||
            (singleCharIdentifierPercent > IDENTIFIER_THRESHOLD && tokenDensity > IDENTIFIER_THRESHOLD) ||
            linesOfCode > LOC_THRESHOLD);

    if (isGenerated) {
        // eslint-disable-next-line no-console -- marvin-js
        console.log(`Token Count: ${tokens.length}, LOC: ${linesOfCode}, Token Density: ${tokenDensity}, ID(%): ${singleCharIdentifierPercent}`);
    }
    return isGenerated;
}
...
function parse(text, languageOptions, filePath) {
    ...
    try {
        ...
        // marvin-js >
        if (isGeneratedAST(ast)) {
            const error = new Error(`Excluding ${filePath} from analysis since it seems to be a generated file`);
        ...
        // marvin-js
        const message = ex.code === 400 ? ex.message : `Parsing error: ${ex.message.replace(/^line \d+:/iu, "").trim()}`;
