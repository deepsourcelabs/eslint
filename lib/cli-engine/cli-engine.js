/* eslint-disable internal-rules/multiline-comment-style -- marvin-js */
/* eslint-disable no-console -- marvin-js */
/**
...
/**
 * marvin-js
 * Group and count the errors for logging.
 * The ruleId is used as key for count. If ruleId is null then the message is used as key
 * @param {Object} messages errors (messages) reported by eslint
...
function verifyText({
    ...
    // Log below information to help in debugging - marvin-js
    const groupedErrors = groupErrors(messages);
    const stats = { ...calculateStatsPerFile(messages) };

    console.log(`\nFile: ${filePath}`);
    console.log(`Lines: ${text.split(/\r\n|\r|\n/u).length}`);
    console.log(`Errors: ${JSON.stringify(groupedErrors)}`);
    ...
    // Commented to prevent the source code from being appended to the result object - marvin-js
    // if (
